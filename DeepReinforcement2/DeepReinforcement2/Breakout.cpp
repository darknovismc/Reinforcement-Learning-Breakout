...
Breakout::Breakout():oldTimeSinceStart(0) 
{
    init();
}
...
void Breakout::display(void) {
    ...
    switch (gameState) {
        ...
        case Gameplay:
            // Draw the game
            drawGame();
			paddle.xpos += paddleMove*deltaTime*2000.0f;
			if (paddle.xpos < 0)
				paddle.xpos = 0;
	...
	glutTimerFunc((int)TIMER, recomputeFrame, 0);
    glutSwapBuffers();
}
...
void Breakout::newBall(float x = -1, float y = -1) {
    Ball b1;
    b1.xpos = (float)rand() / (RAND_MAX)*(WINWIDTH/8*7)+50;
    b1.ypos = WINHEIGHT - 40.0f;
...
void Breakout::drawBalls(void) {
    for (std::vector<Ball>::iterator it = balls.begin(); it != balls.end(); ) {
        ...
        for (std::vector<Brick>::iterator br = bricks.begin(); br != bricks.end(); ) {
            ...
            d = powf((it->xpos - br->xpos), 2.0) + powf((it->ypos - br->ypos), 2.0);
            if (d < it->radius + 5.0) {
                it->xvel *= -1;
                it->yvel *= -1;
                br = hitBrick(br);
            ...
            d = powf((it->xpos - br->xpos - br->width), 2.0) + powf((it->ypos - br->ypos), 2.0);
            if (d < it->radius + 5.0) {
                it->xvel *= -1;
                it->yvel *= -1;
                br = hitBrick(br);
            ...
            d = powf((it->xpos - br->xpos), 2.0) + powf((it->ypos - br->ypos - br->height), 2.0);
            if (d < it->radius + 5.0) {
                it->xvel *= -1;
                it->yvel *= -1;
                br = hitBrick(br);
            ...
            d = powf((it->xpos - br->xpos - br->width), 2.0) + powf((it->ypos - br->ypos - br->height), 2.0);
            if (d < it->radius + 5.0) {
                it->xvel *= -1;
                it->yvel *= -1;
                br = hitBrick(br);
		...
		{
			float yDstPaddleToBall = paddle.ypos - (it->ypos + it->radius);
            if (yDstPaddleToBall<10 && yDstPaddleToBall >= 0)
			{
				if(it->yvel>0)
					it->yvel *= -1;
                reward = 100;
...
void Breakout::specialKeyPress(int key, int x, int y) 
{
    switch(key)
	{
		case GLUT_KEY_LEFT:
...
void Breakout::specialKeyRelease(int key, int x, int y) 
{
    switch(key)
	{
		case GLUT_KEY_LEFT:
