...
TensorflowModel::TensorflowModel()
{
	...
	batchesNum = 1;
	predictedIndices = new int[batchesNum];
	predictions = new float[targetDataSize * batchesNum];
	...
	readWtDense0_op = TF_GraphOperationByName(graph, "dense/kernel/read");
	readWtDense1_op = TF_GraphOperationByName(graph, "dense_1/kernel/read");
	readWtDense2_op = TF_GraphOperationByName(graph, "dense_2/kernel/read");
	assignWtDense0_op = TF_GraphOperationByName(graph, "dense/kernel/Assign");
	assignWtDense1_op = TF_GraphOperationByName(graph, "dense_1/kernel/Assign");
	assignWtDense2_op = TF_GraphOperationByName(graph, "dense_2/kernel/Assign");

	readBsDense0_op = TF_GraphOperationByName(graph, "dense/bias/read");
	readBsDense1_op = TF_GraphOperationByName(graph, "dense_1/bias/read");
	readBsDense2_op = TF_GraphOperationByName(graph, "dense_2/bias/read");
	assignBsDense0_op = TF_GraphOperationByName(graph, "dense/bias/Assign");
	assignBsDense1_op = TF_GraphOperationByName(graph, "dense_1/bias/Assign");
	assignBsDense2_op = TF_GraphOperationByName(graph, "dense_2/bias/Assign");
}
...
void TensorflowModel::Checkpoint(int type) 
{
	...
	const TF_Operation* op[1] = {type == SAVE ? save_op: restore_op};
...
int* TensorflowModel::Predict(const float* inputData, int batchSize)
{
	// batch consists of 4 2x1 matrices.
	const int64_t dims[2] = { batchSize, inputDataSize};
	...
	TF_DeleteTensor(t);
	Okay();
	int outputTensorSize = targetDataSize*sizeof(float)*batchSize;
	...
	if (batchSize != batchesNum)
	{
		batchesNum = batchSize;
		delete[] predictedIndices;
		predictedIndices = new int[batchSize];
		delete[] predictions;
		predictions = new float[targetDataSize * batchSize];
	}
	memcpy(predictions, TF_TensorData(output_values[0]), outputTensorSize);
	TF_DeleteTensor(output_values[0]);
	int idxPred =0;
	float maxPred = predictions[0];
	for (int j = 0; j < batchSize; j++)
	{
		maxPred = predictions[j * targetDataSize];
		for (int i = 0; i < targetDataSize; i++)
		{
			if (predictions[i + j * targetDataSize] > maxPred)
			{
				maxPred = predictions[i + j * targetDataSize];
				idxPred = i;
			}
		}
		predictedIndices[j] = idxPred;
	}
